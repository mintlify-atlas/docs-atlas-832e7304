...
curl -X GET "http://localhost:8080/tenant/info?id=1" \
...
<CodeGroup>
```bash Request Example
curl -X GET "http://localhost:8080/tenant/list?search={\"type\":\"1\"}" \
  -H "Authorization: Bearer {token}"
```
...
<CodeGroup>
```bash Request Example
curl -X GET "http://localhost:8080/tenant/checkIsNameExist?id=0&name=test_tenant" \
  -H "Authorization: Bearer {token}"
```
