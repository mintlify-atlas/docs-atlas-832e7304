...
<CodeGroup>
```bash cURL
curl -X POST "http://localhost:8080/supplier/add" \
...
<CodeGroup>
```bash cURL
curl -X POST "http://localhost:8080/supplier/findBySelect_cus" \
  -H "Content-Type: application/json" \
  -d '{
    "key": "ABC",
    "limit": 20
...
<CodeGroup>
```bash cURL
curl -X POST "http://localhost:8080/supplier/importVendor" \
  -F "file=@vendors.xlsx"
```
